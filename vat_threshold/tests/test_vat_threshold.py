...
class TestVatThreshold(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.custom_module_category = cls.env.ref(
    ...
    def test_vat_threshold_group_is_assignable_custom_module_permission(self):
        user = self._make_internal_user(company=self.env.company, prefix='vat_assignable')

        self.assertEqual(self.group_vat_threshold.privilege_id.category_id, self.custom_module_category)
        self.assertNotIn(self.group_vat_threshold, user.custom_module_group_ids)

        user.write({'custom_module_group_ids': [Command.set([self.group_vat_threshold.id])]})
        self.assertIn(self.group_vat_threshold, user.group_ids)
        self.assertEqual(user.custom_module_group_ids, self.group_vat_threshold)

        user.write({'custom_module_group_ids': [Command.clear()]})
    ...
    def test_vat_threshold_menu_and_operator_actions_use_vat_group(self):
        parent_menu = self.env.ref('Audit_Report.account_report_menu')
