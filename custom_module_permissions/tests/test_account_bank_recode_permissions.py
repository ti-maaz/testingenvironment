...
class TestAccountBankRecodePermissions(TransactionCase):
    def _bank_recode_actions_available(self):
        return all(
    ...
    def test_group_and_visibility_bindings(self):
        group = self.env.ref('custom_module_permissions.group_custom_module_account_bank_recode')
        ...
        if not self._bank_recode_actions_available():
            self.skipTest('account_bank_recode actions are not available in this database.')
    ...
    def test_post_init_grants_group_to_internal_users(self):
        user = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'bridge_bank_recode_user',
            'login': 'bridge_bank_recode_user',
